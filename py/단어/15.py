...
win=[0,0,0]
try:
    with open("단어.txt", 'r', encoding='utf-8') as file:
        for line in file:
            # 줄바꿈 문자 제거 및 '/'로 분리
        ...
        print("Error: File '단어.txt' not found.")
except IOError:
        print("Error: Could not read file '단어.txt'.")

def check_answer(user_input, answers):
    user_answers = sorted(set(a for a in user_input.replace(' ', '').split(',')))
    nanswers = sorted(set(a for a in answers.replace(' ', '').replace('.', '').replace('~', '').split(',')))
    win[1]+=1
